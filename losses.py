...
class CELossSame(nn.Module):
    ...
    def forward(self, outputs, target):
        ...
        target = target.reshape(-1).long()  # (*)
        return (
            self.CELoss(output_img, target)
            + self.CELoss(output_txt, target)
            + self.CELoss(output_sen, target)
        )
...
class CELossTotal(nn.Module):
    ...
    def forward(self, output, target):
        return self.CELossShift(output[0], target[0]) + self.CELoss(
            output[1], target[1]
        )
...
class CELossTotalEval(nn.Module):
    ...
    def forward(self, output, target):
        return (
            self.CELossShift(output[0], target[0])
            + self.CELoss(output[1], target[1])
            + self.CELoss(output[2], target[1])
        )
